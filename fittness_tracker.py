...
def mouse_callback(event, x, y, flags, param):
    ...
    if app_mode == "HOME":
        ...
        skip_btn_w_r,skip_btn_h_r=180,BUTTON_HEIGHT;skip_btn_x_r=canvas_w//2-skip_btn_w_r//2;skip_btn_y_r=canvas_h//2+int(LARGE_TIMER_SCALE*25)
...
def draw_chat_ui(canvas):
    ...
    chat_total_content_height = current_y_on_all_msg_canvas; max_allowed_scroll_up = max(0, chat_total_content_height - chat_visible_area_height); chat_scroll_offset_y = np.clip(chat_scroll_offset_y, 0, max_allowed_scroll_up); src_y1 = int(max(0, (chat_total_content_height - chat_visible_area_height) - chat_scroll_offset_y)); src_y2 = int(min(src_y1 + chat_visible_area_height, ALL_MESSAGES_CANVAS_HEIGHT))
                ...
                # === OTHER EXERCISES (SQUAT, PUSH UP, PULL UP, DEADLIFT) ===
                    ...
                    if current_exercise == "SQUAT":
                        ...
                        # Body straightness (SHA) check above will use available landmarks or default.
                         ...
                         visible_check = all(p[3] > 0.5 for p in [el_l, el_r, sh_l, sh_r, hip_l, hip_r]) # Need elbows, shoulders, hips for core movement and rep counting
